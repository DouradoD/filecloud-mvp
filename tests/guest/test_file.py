...
def test_get_file_list(config_and_login):
...
def test_file_exists(config_and_login):
    config, session = config_and_login
    file = File(session=session, user_type=config['user_type'], base_url=config["base_url"])
...
def test_get_file_info(config_and_login):
    config, session = config_and_login
    file = File(session=session, user_type=config['user_type'], base_url=config["base_url"])
    path = "/diogoaugustodourado/My Files"
...
def test_download_file(config_and_login):
    config, session = config_and_login
    file = File(session=session, user_type=config['user_type'], base_url=config["base_url"])
    params = {
...
def test_upload_file(config_and_login):
    config, session = config_and_login
